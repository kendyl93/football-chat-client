...
export const useSingleMatchScreen = (params: any, userName: string) => {
  ...
  const [messaginginputContainerHeight, setMessaginginputContainer] =
    useState(0);
  ...
  useLayoutEffect(() => {
    socket.emit(
      "join-room",
      id,
      async () => await getRoomMessages(id, setChatMessages)
    );
  }, []);
  ...
  return {
    chatMessages,
    messaginginputContainerHeight,
    setMessaginginputContainer,
    handleNewMessage,
    message,
    setMessage,
  };
